...
ensure_base_packages(){
  local missing=()
  for x in git curl flock python3; do have "$x" || missing+=("$x"); done
  if ((${#missing[@]})); then
    apt-get update
    DEBIAN_FRONTEND=noninteractive apt-get install -y git curl ca-certificates util-linux python3
  fi
  have node || die "Node.js >=18 não encontrado"
  have npm || die "npm não encontrado"
  local major
  major="$(node -p 'Number(process.versions.node.split(".")[0])')"
  (( major >= 18 )) || die "Node.js >=18 necessário; atual=$major"
}

ensure_checkout(){
  if [[ -d "$APP_DIR/.git" ]]; then return; fi
  if [[ "$SCRIPT_DIR" != "$APP_DIR" && -d "$SCRIPT_DIR/.git" ]]; then
    APP_DIR="$SCRIPT_DIR"
    return
  fi
...
migrate_legacy_state(){
  ...
  log "Detectada instalação legada em $LEGACY_DIR"
  systemctl stop "$LEGACY_SERVICE" 2>/dev/null || true

  if [[ -d "$LEGACY_DIR/.wwebjs_auth" ]]; then
    log "Migrando sessão WhatsApp"
    copy_tree "$LEGACY_DIR/.wwebjs_auth" /var/lib/whatsnotify/sessions
  fi
  if [[ -d "$LEGACY_DIR/.wwebjs_cache" ]]; then
    log "Migrando cache WhatsApp Web"
    copy_tree "$LEGACY_DIR/.wwebjs_cache" /var/lib/whatsnotify/web-cache
  fi
  if [[ -d "$LEGACY_DIR/.cache/puppeteer" ]]; then
    log "Migrando cache Puppeteer"
    copy_tree "$LEGACY_DIR/.cache/puppeteer" /var/lib/whatsnotify/puppeteer-cache
  fi
...
health_check(){
  local attempts="${UPDATE_HEALTH_ATTEMPTS:-45}" delay="${UPDATE_HEALTH_DELAY:-2}"
  local url="${UPDATE_HEALTH_URL:-http://127.0.0.1:8080/api/health}"
  local i
...
install_or_migrate(){
  ...
  [[ "${DASHBOARD_PASSWORD:-CHANGE_ME_LONG_RANDOM_PASSWORD}" != "CHANGE_ME_LONG_RANDOM_PASSWORD" ]] || log "WARN DASHBOARD_PASSWORD ainda usa valor padrão"
  install_dependencies
  deploy_systemd
  if ! activate_new_service; then
    log "Nova instalação falhou no health check"
    if [[ "$legacy" == true ]]; then
      log "Reativando serviço legado"
      systemctl stop "$SERVICE" 2>/dev/null || true
      systemctl enable --now "$LEGACY_SERVICE" 2>/dev/null || true
    fi
    exit 20
  fi
  log "WhatsNotify instalado e validado"
  status_mode
}
...
acquire_update_lock(){
  ...
  touch "$LOG_FILE"
  exec 9>"$LOCK_FILE"
  flock -n 9 || die "Atualização já em execução"
}
...
check_mode(){
  ...
  load_env
  cd "$APP_DIR"
  local remote="${AUTO_UPDATE_REMOTE:-origin}" branch="${AUTO_UPDATE_BRANCH:-main}"
...
do_update(){
  ...
  update_common
  local remote="${AUTO_UPDATE_REMOTE:-origin}" branch="${AUTO_UPDATE_BRANCH:-main}"
  local interval="${AUTO_UPDATE_INTERVAL:-300}"
  if [[ "$auto" == true ]]; then
    [[ "${AUTO_UPDATE_ENABLED:-true}" == true ]] || exit 0
    local now last=0
    now="$(date +%s)"
    if [[ -f "$STATE_FILE" ]]; then
      last="$(python3 -c 'import json,sys; print(int(json.load(open(sys.argv[1])).get("lastCheckEpoch",0)))' "$STATE_FILE" 2>/dev/null || echo 0)"
    fi
...
rollback_mode(){
  ...
  if [[ -z "$target" && -f "$STATE_FILE" ]]; then
    target="$(python3 -c 'import json,sys; print(json.load(open(sys.argv[1])).get("previousCommit", ""))' "$STATE_FILE" 2>/dev/null || true)"
  fi
...
status_mode(){
  ...
  if [[ -d "$APP_DIR/.git" ]]; then
    say "Commit: $(git -C "$APP_DIR" rev-parse --short=12 HEAD 2>/dev/null || echo unknown)"
  fi
  ...
  if [[ -r "$ENV_FILE" ]]; then
    load_env
    say "Dashboard: http://${DASHBOARD_BIND:-127.0.0.1}:${DASHBOARD_PORT:-8080}/"
  fi
...
install   Instala do zero ou migra automaticamente /opt/whatsapp-forwarder.
migrate   Alias de install; detecta e preserva sessão/configuração legada.
update    Atualiza para origin/main com health check e rollback automático.
check     Apenas verifica se existe nova versão.
rollback  Retorna ao previousCommit ou ao SHA informado.
repair    Reaplica dependências, permissões e units systemd.
status    Exibe estado resumido da instalação.
EOF
    exit 2
    ;;
esac
